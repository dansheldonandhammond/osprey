			...
			this.dispatchEvent(
				new CustomEvent("slideChanged", {
					detail: {
						...
						currentElement:
							this.sliderItemsToShow[this.currentPage - 1],
					},
				})
			);
		}
	...
	updateSelectionMetadata({ target }) {
		...
		if (tagName === "SELECT" && target.selectedOptions.length) {
			...
			if (swatchValue) {
				...
				selectedDropdownSwatchValue.classList.remove(
					"swatch--unavailable"
				);
			} else {
				selectedDropdownSwatchValue.style.setProperty(
				...
				selectedDropdownSwatchValue.classList.add(
					"swatch--unavailable"
				);
			}
			...
			selectedDropdownSwatchValue.style.setProperty(
				"--swatch-focal-point",
				target.selectedOptions[0].dataset.optionSwatchFocalPoint ||
					"unset"
			);
